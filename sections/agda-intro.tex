...
Another thing to note is that this is the first \emph{multi-argument} function
we have defined.
Multiple arguments in Agda are represented using currying: this is why the type
signature is \(\agdambb{N} \to \agdambb{N} \to \agdambb{N}\).
Instead of writing a function which ``takes two \agdambb{N}s and returns one'',
we write a function which ``takes an \agdambb{N} and returns a function which
takes ''
