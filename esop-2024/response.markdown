...
>ndeed, there is significant overlap with the early part of the paper and other
>ork (though we would point to Frumin, D., Geuvers, H., Gondelman, L., van der
>eide, N.: Finite Sets in Homotopy Type Theory [2018] as the closest work).
>he difference with our work is the new setting of cubical type theory, which is
>hat facilitates the novel theorems later on (theorem 2.7, 3.4, and the library
>evelopment as a whole).

>owever, in the interests of space, we are happy to cut down the exposition of
>hese early theorems and instead present them from a high level, with pointers
>o their original proofs.

>fter converting the paper to the LNCS format it stands at 30 pages, without
>ny editing or squeezing, so we think it should be possible to get down to 25
>ages with this change.
...
>ndeed, it is not possible to prove functions with cardinality other than 0 are
>inite without extensionality.
...
>e will emphasise this contribution more clearly in an update.
...
>ndeed, this is an oversight on our part.
>riginally, the type in question was meant to contain a function: the
>somorphism between `Fin`s, used to represent permutations.
>owever, as noted in the paper, this type doesn't work for efficiency reasons,
>o we used a different representation of permutations.
>issing from the paper (but to be included in an update) is a proof that the two
>epresentations are isomorphic, and therefore equivalent, resulting in a
>initeness proof over a type which *does* contain a function.
...
>e disagree.
>hile it is certainly not an efficient solution, semantically speaking the
>ssence of the countdown problem is one of constraint satisfaction, and 
>roving or disproving that a system of constraints is satisfiable amounts to
>aking a judgement about the cardinality of the set of possible solutions.
>hat is *precisely* what our work does.
>he question "does this countdown problem have a solution" is a question about
>he cardinality of the type representing its solutions.
> constructive answer to that question is a precise enumeration of that type.

>o be clear, we don't imagine people using the library to solve problems like
>he countdown problem.
>e think that the finiteness proofs and explanations are useful for other
>ormalisation efforts
...
>he fully new proofs in the paper are:
...
>f course, these theorems have some sub-lemmas that are also new (and others
>hat have been proven before); we will identify which is which inline in an
>pdate.
...
>he example in the paper takes a few seconds to typecheck.

> 4. How would you plan to shrink this paper to 25 LNCS pages?

>s suggested, we will shorten the discussion of theorems already proven
>lsewhere, and instead focus on a high-level summary of the classification of
>redicates, and leave the in-depth proofs for the novel work
